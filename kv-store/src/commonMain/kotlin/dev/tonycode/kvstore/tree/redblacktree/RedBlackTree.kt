...
import dev.tonycode.kvstore.tree.Tree


// origin: https://github.com/aostrouhhov/trees/blob/master/src/redblacktree/RedBlackTree.kt
...
class RedBlackTree<K: Comparable<K>, V>: Tree<K, V>, Iterable<Pair<K, V>> {
    ...
    override fun iterator(): Iterator<Pair<K, V>> {
        return (object: Iterator<Pair<K, V>> {
            var node = max(root)
            var nextNode = max(root)
            val lastNode = min(root)

            override fun hasNext(): Boolean {
                return node != null && node!!.key >= lastNode!!.key
            }
    ...
    fun clone(): RedBlackTree<K, V> {
        val cloned = RedBlackTree<K, V>()
