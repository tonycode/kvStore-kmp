package dev.tonycode.kvstore.tree.redblacktree


// origin: https://github.com/aostrouhhov/trees/blob/master/src/redblacktree/RBNode.kt
// with some adaptations for KMP and clone() method
data class RBNode<K: Comparable<K>, V>(
    var key: K,
    var value: V,
    var parent: RBNode<K, V>? = null,
    var colorBlack: Boolean = false,
    ...
    fun clone(): RBNode<K, V> {
        val clonedLeft = left?.clone()
        val clonedRight = right?.clone()

        val cloned = RBNode(key, value, null, colorBlack, clonedLeft, clonedRight)
        clonedLeft?.parent = cloned
        clonedRight?.parent = cloned
